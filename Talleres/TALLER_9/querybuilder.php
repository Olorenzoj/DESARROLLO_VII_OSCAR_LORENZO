...
//filtrado
$filter = new QueryBuilder($pdo);

$resultados = $filter->table('productos')
    ->select('id', 'nombre', 'precio', 'categoria_id', 'disponibilidad')
    ->where('precio', '>=', 50)  // Filtrar por precio mínimo
    ->where('categoria_id', '=', 2)  // Filtrar por categoría
    ->where('disponibilidad', '=', 1)  // Filtrar por disponibilidad
    ->orderBy('precio', 'ASC')  // Ordenar por precio ascendente
    ->execute();
...
//reportes
$reporte = new QueryBuilder($pdo);

$resultados = $reporte->table('ventas')
    ->select('id', 'cliente_id', 'producto_id', 'fecha', 'monto')  // Selección dinámica de campos
    ->where('fecha', '>=', '2024-01-01')  // Filtro dinámico por fecha
    ->where('monto', '>', 100)  // Filtro dinámico por monto
    ->groupBy(['cliente_id'])  // Agrupar por cliente
